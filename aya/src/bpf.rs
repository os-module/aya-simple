...
fn detect_features() -> Features {
    ...
    let f = Features::new(true, false, false, true, false, false, false, None);
...
impl<'a> EbpfLoader<'a> {
    ...
    pub fn load(&mut self, data: &[u8]) -> Result<Ebpf, EbpfError> {
        ...
        obj.maps.iter().for_each(|(name, map)| {
            info!("Loading map {name}, data size {}", map.data().len());
        });
        let btf_fd = if let Some(features) = &FEATURES.btf() {
            if let Some(btf) = obj.fixup_and_sanitize_btf(features)? {
                info!("BTF fixup and sanitize done");
        ...
        for (name, mut obj) in obj.maps.drain() {
            ...
            info!("Create kernel map for  map {name} of type {map_type:?}");
            ...
            if let Some(max_entries) = max_entries_override(
                ...
                info!("Overriding max_entries for map {name} to {max_entries}");
        ...
        info!("Try to relocate maps and calls");
        obj.relocate_maps(
            maps.iter()
        ...
        let programs = obj
            ...
            .map(|(name, prog_obj)| {
                ...
                let program = if extensions.contains(name.as_str()) {
                    ...
                    match &section {
                        ProgramSection::KProbe => {
                            info!("Loading KProbe program {name}");
