use alloc::format;
use std::{io, os::fd::BorrowedFd, path::Path};
...
pub(crate) fn get_fdinfo(fd: BorrowedFd<'_>, key: &str) -> Result<u32, ProgramError> {
    ...
    // Ok(0)
    unimplemented!("Get the specified information from a file descriptor's fdinfo")
}
...
pub(crate) fn find_tracefs_path() -> Result<&'static Path, ProgramError> {
    ...
    unimplemented!("Find tracefs filesystem path")
}
